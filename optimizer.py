...
import numpy as np
from elipse import asa_eliptica
import random
import models
# from random import *
...
cw = random.uniform(5,15)/100
Span, chord, offset, area, alon = asa_eliptica(cw, 1.2)
b_min_h = 0.2
c_min_h = 0.1
b_max_h = 1.2
c_max_h = 0.4
...
perfis_asa = ['Profile_1', 'Profile_19', 'Profile_20', 'Profile_21']
perfis_eh = ['EH_teste']
...
def gerar_inicial(total):
    ...
    while len(aeronaves) < total:
        ...
        geometria_asa = [(Span[0], chord[0], offset[0]), (Span[1], chord[1], offset[1]), (Span[2], chord[2], offset[2]), (Span[3], chord[3], offset[3]), (Span[4], chord[4], offset[4]), (Span[5], chord[5], offset[5])] #(bint, cint, o1),
        bh = random.uniform(b_min_h, b_max_h)
        ...
        #ht_min = models.zcg+0.04
        lt = random.uniform(cw*4/3, 1.5 - ch)
        ht = (lt*np.tan(np.pi/180*17))
        

        pos_cp = round(random.uniform(pos_cp_min, pos_cp_max)*cw*4, 2)
...
def variar(aeronave, sigma):  # função para variar os paramestros de uma aeronave
    while True:
        ...
        bh = round(trunc_gauss(bh, sigma, b_min_h, b_max_h), 3)
        ...
        ht = round(trunc_gauss(ht, sigma, 0, 4*cw), 3)

        lt = round(aeronave.posicoes['eh'][0], 3)
        lt = round(trunc_gauss(lt, sigma, 4/3*cw, 1.5 - ch), 3)

        pos_cp = round(trunc_gauss(pos_cp, sigma, pos_cp_min*cw*4, pos_cp_max*cw*4), 3)

        geometria_asa = [(Span[0], chord[0], offset[0]), (Span[1], chord[1], offset[1]), (Span[2], chord[2], offset[2]), (Span[3], chord[3], offset[3]), (Span[4], chord[4], offset[4]), (Span[5], chord[5], offset[5])] #(bint, cint, o1), #(bint, cint, o1),
        geometria_eh = [(0, ch, 0), (bh, ch, 0)]
        geometria_ev = [(0, crv, 0), (bv, ctv, crv-ctv)]
...
def verifica_cond(aeronave):
    ...
    if aeronave.altura > altura_max:
        retorno = False
    if aeronave.geometria_asa[5][0] < aeronave.geometria_eh[1][0]:
        retorno = False
    if aeronave.dist_solo_ev < dist_solo_ev_min:
        retorno = False
    if aeronave.posicoes['asa'][1] > aeronave.posicoes['eh'][1]:
        retorno = False
    return retorno
